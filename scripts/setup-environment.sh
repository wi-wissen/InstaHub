#!/bin/bash
#
# Idempotent environment bootstrap for InstaHub (Laravel 13 / PHP 8.5 / Node 24).
#
# It provisions everything needed to run the app, the linter (Pint) and the
# Pest test suite from a clean container:
...
# Called by .claude/hooks/session-start.sh, but can also be run by hand:
